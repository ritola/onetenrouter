...
module.exports = {
  init: function (index, input, output) {
    ...
    return {
      ...
      leds: {
        stanton: (color) => { sendNoteOn(midi.stanton.key, color) },
        deckA: (color) => { sendNoteOn(midi.deckA.key, color) },
        deckB: (color) => { sendNoteOn(midi.deckA.key, color) },
        pitchRed: (color) => { sendNoteOn(midi.pitchRed.key, color) },
        pitchBlue: (color) => { sendNoteOn(midi.pitchBlue.key, color) }
      },
    ...
    function noteAndLed (key) {
      const stream = inStream.filter(R.either(isSpeficicNoteOff(key), isSpecificNoteOn(key)))
