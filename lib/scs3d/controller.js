...
const { isSpecificNote, isSpecificNoteOn, isSpecificNoteOff } = require('../midi-helpers')

module.exports = {
  init: function (index, input, output) {
    const inStream = input.stream.map('.message')
    inStream.onValue(midiHelpers.log)
    ...
    function noteAndLed (key) {
      const onPress = onSpecificNote(key)
      const setLed = (color) => sendNoteOn(key, color)
      const ledOnPress = (up, down) => {
        setLed(up)
        onPress.filter(isSpecificNoteOn(key)).onValue(() => { setLed(down) })
        onPress.filter(isSpecificNoteOff(key)).onValue(() => { setLed(up) })
      }
