...
const { play, cue } = require('../midi-map')
...
module.exports = function (controllers, virtual) {
  ...
  const syncPresses = Bacon.mergeAll(R.map((c) => c.button.sync.onPress, controllers))
  const tapPresses = Bacon.mergeAll(R.map((c) => c.button.tap.onPress, controllers)).filter(R.prop('noteOn'))
  ...
  syncPresses.onValue((x) => {
    const message = x.noteOn ? noteOn : noteOff
    virtual.output.send(message(ch(8 + x.index), note.d8))
  })

  tapPresses.onValue((x) => virtual.output.send(noteOn(ch(8 + x.index), note.dX8)))
